...
@Component
public class DispenseRegisterService extends ReportService<SubReportData<DispenseRegisterData>> {
	@Autowired
	private QueryUtil q;
	...
	public String getName() {
		return "Dispensing Register";
	}

	@Override
	protected Collection<SubReportData<DispenseRegisterData>> generateData(Date from, Date to, String[] deptIds) {
		List<DispenseRegisterData> data = new ArrayList<>();
		...
		this.schema()
				...
				.fetch().forEach((row) -> {
					// @formatter:on
					DispenseRegisterData detail = new DispenseRegisterData();
		...
		List<SubReportData<DispenseRegisterData>> result = new ArrayList<>();

		result.add(new SubReportData<DispenseRegisterData>(data));
	...
	public String getTemplateUrl() {
		return "reports/registers/dispense.jasper";
	}
